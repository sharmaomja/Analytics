import { createDashboard, findDashboardByIdForUser, findSavedQueryByIdForUser, listDashboardWidgetsByDashboardId, listDashboardsByUserId, listDatasetFieldsByDatasetId, } from "../db/queries.js";
import { runDatasetQuery } from "../services/queryService.js";
export async function createDashboardHandler(req, res, next) {
    try {
        if (!req.user) {
...
export async function listDashboardsHandler(req, res, next) {
    try {
        if (!req.user) {
            res.status(401).json({ error: "Unauthorized." });
            return;
...
export async function getDashboardHandler(req, res, next) {
    try {
        ...
        const hydratedWidgets = await Promise.all(widgets.map(async (widget) => {
