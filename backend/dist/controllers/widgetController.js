import { createDashboardWidget, findDashboardByIdForUser, findSavedQueryByIdForUser, deleteDashboardWidgetByIdForUser } from "../db/queries.js";
const allowedChartTypes = new Set(["line", "bar"]);
...
export async function createWidgetHandler(req, res, next) {
    try {
        if (!req.user) {
        ...
        const chartType = String(req.body.chart_type ?? "").trim();
...
export async function deleteWidgetHandler(req, res, next) {
    try {
        if (!req.user) {
            res.status(401).json({ error: "Unauthorized." });
            return;
