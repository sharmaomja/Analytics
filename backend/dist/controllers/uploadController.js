...
export async function uploadCsvHandler(req, res, next) {
    try {
        if (!req.user) {
        ...
        const fileRecord = await createFileRecord(dataset.id, uploadedFile.originalname, uploadedFile.mimetype || "text/csv", rawContent, "uploaded");
