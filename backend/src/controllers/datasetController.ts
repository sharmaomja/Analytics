...
export async function createDatasetHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) {
...
export async function listDatasetsHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({ error: "Unauthorized." });
      return;
