...
import { createSavedQuery, findDatasetByIdForUser, listDatasetFieldsByDatasetId, type QueryConfig, type QueryFilter } from "../db/queries.js";
import { runDatasetQuery } from "../services/queryService.js";

type ErrorWithStatus = Error & { statusCode?: number };

function normalizeQueryConfig(body: Request["body"]): QueryConfig {
  const dataset_id = String(body.dataset_id ?? "").trim();
  const metrics = Array.isArray(body.metrics) ? body.metrics.map((value: unknown) => String(value).trim()).filter(Boolean) : [];
  const dimensions = Array.isArray(body.dimensions) ? body.dimensions.map((value: unknown) => String(value).trim()).filter(Boolean) : [];
  const filters: QueryFilter[] = Array.isArray(body.filters)
    ? body.filters.map((filterValue: unknown): QueryFilter => {
        const normalized = filterValue as Partial<QueryFilter>;

        return {
          field: String(normalized.field ?? "").trim(),
          op: String(normalized.op ?? "").trim() as QueryFilter["op"],
          value: String(normalized.value ?? "").trim(),
        };
      }).filter((filter: QueryFilter) => Boolean(filter.field && filter.op))
    : [];
...
export async function listDatasetFieldsHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) {
...
export async function runQueryHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({ error: "Unauthorized." });
      return;
...
export async function saveQueryHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({ error: "Unauthorized." });
      return;
